...
/* 地点显隐的缩放门（2026-08-20 用户点单改公里锚定）：按 rank 给「多少公里每像素以内才画」。
   ⚠ 原为绝对度数 [∞,0.2,0.12,0.065,0.045] °/px——同一个阈值在不同星球半径上代表的地面距离并不
   相同（R=10000km 的世界比地球漂 57%），同「笔刷档位必须公里锚定」之训。新值＝旧值按地球口径
   （111.19 km/°）折算后取整到易读档，地球上的显隐时机偏移 ≤2.6%＝不足滚轮一档的 1/7。
   判据 km/px ＝ degPerPx × 每度公里，与画面左下比例尺读数同一口径（投影里两处 cos 正好抵消）。 */
export const RANK_KM_PX = [Infinity, 22, 13, 7.2, 5];
