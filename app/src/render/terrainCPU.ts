...
export function createTerrainCPU(canvas: HTMLCanvasElement): TerrainRenderer {
  ...
  const elevOf = (r: number, c: number) => field ? field[r * grid!.cols + c] : terrainProps(grid!.cells[r][c]).elev;
  function elevBilinear(lon: number, lat: number): number {
    const g = grid!, fx = (lon - g.bb.lonMin) / g.step - 0.5, fy = (lat - g.bb.latMin) / g.step - 0.5;
    ...
    const top = elevOf(r0, c0) + (elevOf(r0, c1) - elevOf(r0, c0)) * tx, bot = elevOf(r1, c0) + (elevOf(r1, c1) - elevOf(r1, c0)) * tx;
  ...
  function renderTile(bb: BBox, pxpd: number, opts: TerrainRenderOpts): HTMLCanvasElement {
    ...
    for (let y = 0; y < H; y++) for (let x = 0; x < W; x++) {
      const p = L2P(x, y); let e = elevBilinear(p[0], p[1]);
    ...
    for (let y = 0; y < H; y++) for (let x = 0; x < W; x++) {
      ...
      if (e >= -0.02) {
        ...
        if (opts.contour) {   // 等高线：与 GL 版同一带宽公式（aa 用邻差近似 fwidth；等高距可配）
          const ci = opts.cInt || 0.12;
          const aa = Math.abs(eR - e) + Math.abs(eD - e) + 1e-6;
          const lv = (e + 0.02) / ci - Math.floor((e + 0.02) / ci);
          const d2 = Math.min(lv, 1 - lv) * ci / aa;
          const line = 1 - Math.max(0, Math.min(1, (d2 - 0.6) / 0.6));
          col = [col[0] + (90 - col[0]) * line * 0.28, col[1] + (70 - col[1]) * line * 0.28, col[2] + (40 - col[2]) * line * 0.28];
        }
      }
  ...
  return {
    canvas, kind: "cpu",
    uploadGrid(g: Grid, elev?: Float32Array) { grid = g; field = elev || null; tile = null; },
    render(viewBB: BBox, opts: TerrainRenderOpts = {}) {
      ...
      const key = (opts.diag ? "d" : "") + (opts.contour ? "c" + (opts.cInt || 0.12) : "");
